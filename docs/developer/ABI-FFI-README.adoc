...
# CONSENT_AWARE_WEB ABI/FFI Documentation
...
│  generated/abi/consent_aware_web.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libconsent_aware_web.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── consent_aware_web.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── consent_aware_web.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/consent_aware_web.h
```
...
```c
#include "consent_aware_web.h"

int main() {
    void* handle = consent_aware_web_init();
    if (!handle) return 1;

    int result = consent_aware_web_process(handle, 42);
    if (result != 0) {
        const char* err = consent_aware_web_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    consent_aware_web_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lconsent_aware_web -L./zig-out/lib
```

### From Idris2

```idris
import CONSENT_AWARE_WEB.ABI.Foreign
...
```rust
#[link(name = "consent_aware_web")]
extern "C" {
    fn consent_aware_web_init() -> *mut std::ffi::c_void;
    fn consent_aware_web_free(handle: *mut std::ffi::c_void);
    fn consent_aware_web_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = consent_aware_web_init();
        assert!(!handle.is_null());

        let result = consent_aware_web_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libconsent_aware_web = "libconsent_aware_web"

function init()
    handle = ccall((:consent_aware_web_init, libconsent_aware_web), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:consent_aware_web_process, libconsent_aware_web), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:consent_aware_web_free, libconsent_aware_web), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/consent_aware_web.h
   ```
