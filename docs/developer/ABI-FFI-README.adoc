...
# CONSENT_AWARE_HTTP ABI/FFI Documentation
...
│  generated/abi/consent_aware_http.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libconsent_aware_http.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── consent_aware_http.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── consent_aware_http.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/consent_aware_http.h
```
...
```c
#include "consent_aware_http.h"

int main() {
    void* handle = consent_aware_http_init();
    if (!handle) return 1;

    int result = consent_aware_http_process(handle, 42);
    if (result != 0) {
        const char* err = consent_aware_http_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    consent_aware_http_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lconsent_aware_http -L./zig-out/lib
```

### From Idris2

```idris
import CONSENT_AWARE_HTTP.ABI.Foreign
...
```rust
#[link(name = "consent_aware_http")]
extern "C" {
    fn consent_aware_http_init() -> *mut std::ffi::c_void;
    fn consent_aware_http_free(handle: *mut std::ffi::c_void);
    fn consent_aware_http_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = consent_aware_http_init();
        assert!(!handle.is_null());

        let result = consent_aware_http_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libconsent_aware_http = "libconsent_aware_http"

function init()
    handle = ccall((:consent_aware_http_init, libconsent_aware_http), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:consent_aware_http_process, libconsent_aware_http), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:consent_aware_http_free, libconsent_aware_http), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/consent_aware_http.h
   ```
